...
export default {
  controlPanelSections: [{
    ...
    controlSetRows: [[{
      ...
      config: {
        ...
        description: t('The radius (in pixels) the algorithm uses to define a cluster. ' + 'Choose 0 to turn off clustering, but beware that a large ' + 'number of points (>1000) will cause lag.')
      }
    }], ['row_limit'], ['adhoc_filters'], ['custom_filters'], ['groupby']]
  }, {
    label: t('Points'),
    controlSetRows: [[{
      name: 'point_radius',
      config: {
        ...
        default: 'Auto',
        description: t('The radius of individual points (ones that are not in a cluster). ' + 'Either a numerical column or `Auto`, which scales the point based ' + 'on the largest cluster'),
        mapStateToProps: state => ({
    ...
    controlSetRows: [[{
      ...
      config: {
        ...
        label: t('label'),
        default: [],
        description: t('`count` is COUNT(*) if a group by is used. ' + 'Numerical columns will be aggregated with the aggregator. ' + 'Non-numerical columns will be used to label points. ' + 'Leave empty to get a count of points in each cluster.'),
        mapStateToProps: state => ({
          choices: columnChoices(state.datasource)
        })
      }
      ...
      config: {
        ...
        default: 'sum',
        description: t('Aggregate function applied to the list of points ' + 'in each cluster to produce the cluster label.')
      }
    }]]
  }, {
  ...
  controlOverrides: {
    groupby: {
      description: t('One or many controls to group by. If grouping, latitude ' + 'and longitude columns must be present.')
    }
  }
};